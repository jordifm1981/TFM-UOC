...
sauDades = loadSau.sauTotDF


# Descomposició sèrie temporal
descomp_serie = sm.tsa.seasonal_decompose(sauDades['Volum'], model='additive', freq=365)  

var_graf = ((sauDades['Dia'], descomp_serie.observed, 'Volum Sau','$hm^3$'),(sauDades['Dia'], descomp_serie.trend, 'Tendència','$hm^3$'),
               (sauDades['Dia'], descomp_serie.seasonal, 'Component estacionària','$hm^3$'),(sauDades['Dia'], descomp_serie.resid, 'Component residual','$hm^3$'))

loadSau.grafica_series_multi(var_graf)

# Descomposició sèrie temporal
descomp_serie2007 = sm.tsa.seasonal_decompose(sauDades['Volum'][sauDades['Dia']>='2007-10-01'], model='additive', freq=365)  

var_graf2007 = ((sauDades['Dia'][sauDades['Dia']>='2007-10-01'], descomp_serie2007.observed, 'Volum Sau','$hm^3$'),(sauDades['Dia'][sauDades['Dia']>='2007-10-01'], descomp_serie2007.trend, 'Tendència','$hm^3$'),
               (sauDades['Dia'][sauDades['Dia']>='2007-10-01'], descomp_serie2007.seasonal, 'Component estacionària','$hm^3$'),(sauDades['Dia'][sauDades['Dia']>='2007-10-01'], descomp_serie2007.resid, 'Component residual','$hm^3$'))
...
test_stationarity(sauDades['Volum'])
...
plt.subplot(121)
plt.title('Histograma volum Sau')
plt.xlabel('volum ($hm^3$)')
sauDades['Volum'].hist(bins=20)
plt.subplot(122)
plt.title('Funció densitat de probabilitat')
sauDades['Volum'].plot(kind='kde')
plt.xlabel('volum ($hm^3$)')
plt.show()


# Autocorrelació i autocorrelació parcial
lag_acf = acf(sauDades['Volum'], nlags=50)
lag_pacf = pacf(sauDades['Volum'], nlags=50)
...
ax[0].axhline(y=-1.96/np.sqrt(len(sauDades['Volum'])),linestyle='--',color='gray')
ax[0].axhline(y=1.96/np.sqrt(len(sauDades['Volum'])),linestyle='--',color='gray')
...
ax[1].axhline(y=-1.96/np.sqrt(len(sauDades['Volum'])),linestyle='--',color='gray')
ax[1].axhline(y=1.96/np.sqrt(len(sauDades['Volum'])),linestyle='--',color='gray')
...
volumSau = pd.DataFrame(sauDades['Volum'])
volumSau = volumSau.set_index(pd.DatetimeIndex(sauDades['Dia']))
...
for tick in ax.yaxis.get_major_ticks():
    tick.label.set_fontsize(14) 
plt.legend(mesosDFT.columns, loc=4)
...
volumSauTri = pd.DataFrame(sauDades['Volum'])
volumSauTri = volumSauTri.set_index(pd.DatetimeIndex(sauDades['Dia']))
